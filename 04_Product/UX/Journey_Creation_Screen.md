...
## Implemented in code (mockup v14 fidelity pass — 2026-07-10)

`app/src/app/journey/new.tsx` matches mockup v14 screens 05–08 (`04_Product/UX/UX_References/mockups_v14/screen-05.png`…`screen-08.png`). POC scope stays **6 stages** (Name → Your why → Duration & rhythm → Steps → Reminders → Summary) — Privacy & support and public/creator are out of POC scope (`POC_and_MVP_Scope.md`), so this wizard doesn't yet show the mockup gallery's 7th stage.

- **Name stage:** the Name row uses the pencil-edit affordance (tap the pencil to reveal a text input inline, blurs/submits to close); shows "Untitled Journey" in muted text as a placeholder value. **Description** field sits right below it, matching the mockup's "moved here from the step screen" note.
- **Duration & rhythm stage:** three pencil-edit rows — **Duration** (opens `ChoiceChips` inline), **Type** (static "Frequency" label, read-only — no custom-plan editor in POC), **How often** (opens `ChoiceChips` inline, labelled with the existing `Rhythm` options). Selecting a chip closes that row's editor automatically.
- **Step-progress tooltip:** tapping a bar now **reserves real layout space** below the bars (not an absolutely-positioned overlay) so the stage title is pushed down and never covered — closer to the mockup's intent than a literal absolute-position copy, and avoids text collision at small screen heights.
- **Plan the steps stage:** the green "Recommended" card is **informational only** (no inputs inside it, matching the mockup's static guidance copy). Two separate dashed CTAs follow it: **"Add a Starter Step"** (teal, star icon) and **"Add a step"** (coral, plus icon). Tapping "Add a Starter Step" reveals a Starter Step card (title + optional description, with the example copy) that can be removed; this is a small deliberate divergence from a literal mockup reading (which shows an already-filled example row) — it reuses the same add-affordance pattern as regular Steps for consistency and discoverability, since a first-time user has nothing "already there" to tap.
- **Your why stage:** unchanged from the 2026-07-07 decisions above — question → answer-box pairs, then the reminder-line input (0/50 counter) + teal "Add" button, saved lines as grey delete-chips.
- **Footer:** Back/Next are equal-size, Back disabled (35% opacity) on stage 1; prev/next stage names sit above the buttons and blank out at the first/last stage respectively.
