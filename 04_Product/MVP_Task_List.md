...
| **R7** | **Smart Notification Timing — the aggregate** (this is R1, now specified end to end) | 🟡 In progress 2026-08-26 | Decisions D80–D83 closed every open question. **Built so far:** `core/notify/aggregatePlan.ts` — the pure planner that groups a day's smart reminders into at most two sends, three hours apart, each firing at the EARLIEST time in its group and naming the rules it replaces. **Still to build:** partition rules by `mode` in `CommunicationScheduler.planSchedule` (fixed expands exactly as today; smart goes to the planner and produces no per-rule notification); an `AggregateCopyBuilder` implementation over the `notify` namespace (counts + Journey titles, never Step titles); injecting it at the composition root; the `smart` option in the Journey reminder UI; suppression at send time when nothing is pending or the app is foregrounded; and tests for "three Journeys become one send" and "a fixed reminder still fires that day". |
| **R8** | **Every notification in the user's chosen voice** (D84) | ⛔ Ready | Only `reminder` has the four styles today; `allyRequest`, `allyAccepted`, `allyDeclined`, `allyPermissionChanged`, `allyRemoved`, `journeyFrozen`, `journeyResumed`, `journeyCompleted`, `journeyClosed` and `center` are base-only in both languages. The mechanism already exists (`notificationContent.ts` resolves a `_<styleId>` key suffix and falls back), so this is authoring work: roughly twenty strings × four voices × two languages, plus the gendered Hebrew forms where the base has them. Belongs with the Personalized Motivation Engine, which is where "what do we say" is decided. |
| **R9** | **Personalized Motivation Engine** | ⛔ Founder wants it next, after R7 | `PRD/Future/Personalized_Motivation_Engine_PRD.md` is Future Vision with **open product and architecture questions (§11, §12)** — they have to be answered before it can be built. It is the "what do we say" half of the mechanism: choosing the right message for this person and this moment, learning from feedback, never optimising for opens. Needs no new permission and no native build, which is why the founder put it ahead of calendar/location context. |
