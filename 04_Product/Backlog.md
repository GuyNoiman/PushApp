...
| O-26 | **The career expert's questions are hard-coded English** | **Gap, confirmed in code** | `core/learning/domains/career/index.ts` holds `prompt` and `options` as literal English strings (`career.time`, `career.obstacles` and the rest), not locale keys. This is the language leak the partner reported twice — every answer card he listed comes from this file. Spec §4.10 P0. The other three domains are built the same way |
