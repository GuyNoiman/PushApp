...
| Cost measured server-side | **LIVE 2026-09-18** | Migration 0019 applied (price table seeded, 3 models, verified against Google's page) and `gemini-proxy` deployed, so token counts and conversation ids are now recorded. The console's cost card fills as conversations happen. |
...
| **Account restore on a new device** | **FIXED — server half applied 2026-09-18** | Migration 0020 applied and verified on the live project: the `profiles` trigger is in place, the backfill left 0 real users without a profile row, and `account_entry_status()` exists. The client half shipped in update 16. **Still unproven:** no backup has been written yet — confirm by signing in on a device and checking `account_state` has a row. |
