...
import { LlmError, type LlmClient, type LlmRequest, type LlmResult } from '../llm/LlmClient';
 ...
 * requests-per-minute), so a scripted run waits exactly as long as asked. Falls back to a sane
 * default and is clamped so a bad hint can never park the harness for minutes.
 */
...
/**
 * An {@link LlmClient} decorator that absorbs a free-tier RATE LIMIT (HTTP 429) at the SINGLE-CALL
 * level: the wait-out-the-cool-down-and-retry loop lives INSIDE `complete()`, where each call is an
 * idempotent HTTP request that is safe to repeat. This is deliberately NOT done around an
 * orchestrator turn (`chooseBranch`/`respond`), because those are non-idempotent — re-running
 * `chooseBranch` after a 429 threw "A branch was already chosen" and aborted the run on a transient
 * rate limit. A full interview makes more calls than the per-minute cap allows, so this lets a
 * reproducible run ride out each window and complete on the free tier without cost. Bounded
 * (`maxWaits`); any non-429 failure is re-thrown unchanged for the caller to handle/abort.
 */
class RateLimitRetryingLlmClient implements LlmClient {
  constructor(
