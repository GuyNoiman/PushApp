...
| Feature | Verdict | Detail |
|---|---|---|
| Google sign-in on Android | **BROKEN — first device test, 2026-09-16** | The partner's Android stops on the account screen with `DEVELOPER_ERROR`. That is Google refusing the request because no **Android** OAuth client exists in the Google Cloud project (number 872942140221, the one holding the Web client) for package `com.guynoiman.pushapp` with the SHA-1 of the EAS signing keystore. Configuration only: no build or update needed. Needs the founder (credentials owner). Because the account screen is mandatory, **the first run is completely blocked on Android** until this is done. |
| Sign-in error shown to the person | **BROKEN** | The raw library string ("DEVELOPER_ERROR: Follow troubleshooting instructions at https://…") is printed on the account screen. It must be a human sentence with Try again; the reason stays in the closed KPI bucket. |
| Language direction on first run | **BROKEN** | Same screenshot: English copy laid out right-to-left (right-aligned text, back chevron on the right). The language step flips direction only after a restart, asked through a dialog whose "Not now" leaves the whole first run mirrored — and where restart is unavailable no dialog appears at all. |
