...
| **KPIs** | Empty by design. Its data needs a separate product-analytics consent (§11.2) that nobody has been asked for. `kpi_events` accepts rows and the app sends none. **2026-09-16:** two first-run cards lead the tab — the onboarding funnel (installations per step, and the loss since the step before) and sign-in per provider (attempts · successes · cancels · failures by reason, plus the account step's no-provider escape). They read the same `kpi_counts` and need no migration; they appear only after this folder is redeployed, and count only from app builds carrying taxonomy events added that day. |
