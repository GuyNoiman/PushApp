...
| O-24 | **Nothing hands back what the coach understood before a Journey is created** | Gap | *(Corrected 2026-09-06: the row previously said our build wait is a spinner. It is not — `createJourneyFromGoalSpec` is synchronous and local, so there is no wait at all. The gap is bigger than a loading screen.)* Xoltar spends its build time proving it listened: the person's own facts, then their real OBSTACLE, escalating (`05_Research/Xoltar_Wish_Competitive_Analysis_2026-08-28.md` §1B.5). We have the understanding in `goalSpec` and show none of it. Same item as the starting-point summary, spec §4.8 |
| O-25 | **CTAs are written in the product's voice** | **Half** | Continue · Next · Save. Theirs are the user's own words — I'm in · I want that · That's right · But here's the thing. Cheapest change on the list, and it is the difference between a form and a conversation (§1B.6). First one landed 2026-09-06: the acknowledgement screen's button is "That sounds about right" |
| O-28 | **The coach composes its turns instead of selecting them** | **Shipped 2026-09-06** | `core/coach/composeTurn.ts`. Every turn's TEXT is written by the model from the character plus what is already known; the engine still decides what to ask and when the interview ends. Any failure returns the canned line, so the offline path is unchanged. Fixes findings 1, 2, 5, 9 and most of 14; **does not** fix 3, 6, 7, 10 or 12, which are the engine's decisions rather than its words |
| O-29 | **The engine walks a list; there should not be a list** | **Shipped 2026-09-14** | **Reframed by the founder on 2026-09-14 (D103), and the reframing is bigger than the fix I had planned.** I was going to split the one array into two phases; he removed the array. The engine holds a SET of facts with no order; after every message the whole set is re-read against everything said, and what has been answered is struck off whether or not it was asked. The next turn is chosen by *which would feel most natural now and most improve the plan*, never by position. Plus four questions of room to wander off-slate, and a budget that switches to a cheaper MODEL rather than shortening the understanding. Closes findings 6, 7 and 12 together. **Built:** `core/coach/readConversation.ts` reads the whole transcript against the whole outstanding set after every message and strikes off what was answered, asked or not; `questionIndex` is gone and `outstanding()` is a filter over what is unanswered. Exact-precision axes (weekly time, horizon, scheduling) are struck off only on an exact option match, everything else may be read from ordinary language |
| O-26 | **The career expert's questions are hard-coded English** | **Fixed 2026-09-07** | `core/learning/domains/career/index.ts` holds `prompt` and `options` as literal English strings (`career.time`, `career.obstacles` and the rest), not locale keys. This is the language leak the partner reported twice — every answer card he listed comes from this file. Spec §4.10 P0. Prompts and options are locale lookups now, read when the question is ASKED so a language change mid-interview carries through the rest of it; the feasibility notes and default Step titles came with them. **The other three domains are still built the same way** and need the same pass |
| O-27 | **A question was asked in another question's words** | **Fixed 2026-09-07** | **Reproduced from the founder's recording, and it was one line.** `metaVoiced` looked its prompt up as `` `interview.${intent}` ``, which is correct only while no two questions share an intent — and the horizon question carried `intent: 'time'`. So "How much time can you realistically give this each week?" was rendered with *About a month · About two months · No fixed end*, and the same sentence appeared twice in one interview because both questions were `time`. **Findings 10 and 11 were the same bug.** Three axes now have three names (`time` per week, `horizon` how long, `scheduling` which days), the voicing is an explicit map, and `questionAxes.test.ts` fails if any interview ever assembles two questions on one axis |
| O-31 | **The Users page lists as well as searches** | **Shipped 2026-09-08** | Migration 0017: `admin_list_users` (admin only, capped at 100, keyset-paginated, one audit row per page) and `admin_count_users`. The console opens on the newest accounts and typing narrows to the search. The 0014 note that this is "a search, not a directory" is superseded — a search cannot answer "how many people are on this?" because you have to know a name to ask one |
| O-32 | **The Versions tab shows the update number** | **Shipped 2026-09-08** | Migration 0018 adds `runtime_installs.ota_version`; the app reports it and the tab shows a range per runtime (`updates 3–5`) because two phones on one build sitting on different updates is the thing the tab exists to surface. It read `runtime ad3ed4b6 · 1 install · 1.0.0 · preview` — four identifiers, none of which moves between over-the-air updates |
| O-33 | **The conversation was closed by default and opened only while cheap** | **Fixed 2026-09-08** | D102 says open by default, narrowing to closed choices once the conversation budget is crossed. The build had it backwards: every question shipped cards and the budget withdrew the free-text escape, so the further somebody got the more it behaved like a form. Now the composer bar carries every answer while the budget is `open`; cards appear once it narrows, and for a genuine pick (`allowOther: false`) which free text cannot answer. `answerShape.test.ts` pins the rule |
| O-30 | **There is no registration step in the first run** | **Open — a founder decision, 2026-09-07** | Sign-in exists (`/sign-in`, Apple + Google) but lives in Settings and is optional by design: the app works fully anonymous and the screen says so. The founder wants a sign-in/registration step in onboarding. **It conflicts with a decision already made** — value before account creation — which the Xoltar analysis §1.4 also singles out as something they do well. The open question is not whether but WHERE, and whether manual email/password is in scope at all (it adds verification, reset, and a password surface the product does not have today) |
...
| C-03 | The structure builder: Milestones, Steps, dependencies, release rules | Open | Nothing, deliberately. The largest open design in the PRD; `journey_templates` has no structure column, not even an empty one. Step types are S-22 and are part of this design, not after it |
...
| **S-22** | **Step types** (`PRD/Step_Types_PRD.md`) | A Step is a title, a description and a cadence, and always an instruction to act in the world. It cannot be a questionnaire, a video, an audio piece or an image sequence — so a creator can only author a list of instructions, an expert cannot put its own questions inside a Journey, and `Step.appLink` (D95) stands as a one-off instead of the first case of a general model. Founder is designing it with Codex. **The creator half is the same design as S-13, not a second one.** Nine open questions are named in the PRD; the two that decide the shape are what "done" means for a video without measuring people, and where media lives and what egress costs |
