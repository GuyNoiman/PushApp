...
| O-27 | **No test stops a question rendering another axis's answers** | Gap | Spec §4.9 asks for a deterministic pairing test: a capacity question id may render only capacity answers, a horizon id only horizon answers, and the same mapping must hold in both languages. `general.time` and `career.time` are correctly paired today and `shared.horizon` is a separate question — but nothing prevents the next edit from crossing them |
