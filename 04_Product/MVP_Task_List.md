...
| **R2** | **Journey resume / re-plan — its own PRD** | ✅ **Written 2026-08-27** | `PRD/Journey_Resume_Replan_PRD.md`. One PRD, one feature, governing BOTH the manual pause/resume (J3) and the inactivity freeze/return (J5) — which shared a code path and had no document between them. Everything in it was already decided by the founder on 2026-08-14; the work was extracting it from inside the postponement PRD's §14 Q5, reorganising it as a feature rather than an answer, and **verifying field by field what actually exists in the code** — the answer being nothing: no `Journey.frozenAt`, no extension ledger, no `rebasePlan`, no `JourneyReplanned`, no `pauseReason`. The postponement PRD keeps the full argument (including the superseded freeze-credit design and why it was wrong) and now points here. Two small questions stay OPEN and neither blocks a first slice: whether to add genuinely Journey-level reasons to the one reason config, and whether the early-start rebase should get the same weekday snap. **Ends with a six-step build order.** |
...
| **R4** | **Public privacy policy** | 🟡 **Written 2026-08-27 — five founder blanks left** | `04_Product/Privacy_Policy.md` is the canonical text; it is LIVE in Hebrew and English at https://pushapp-invite.expo.app/privacy.html, and the store-required account-deletion page is live at https://pushapp-invite.expo.app/delete-account. Written from `Privacy_Contract_With_The_User.md`, checked against the section structure of a serious policy, and it covers every clause on the contract's own §4 checklist. **What is left is his, not mine:** the controller entity, a support email a human reads, the minimum age, how long the coach's behavioural log is kept, and the Mirror provider's region. All five are marked `[[…]]` and highlighted yellow on the page so they cannot ship unnoticed. Two things must also become TRUE in the app before publication: the profile blob (which holds a birth date) is encrypted or stops holding a full date, and in-app account deletion is verified end to end against the live database. |
| **R7** | **Smart Notification Timing — the aggregate** (this is R1, specified end to end) | ✅ **Built 2026-08-26** | The planner landed first (`core/notify/aggregatePlan.ts`); this session wired the rest. `CommunicationScheduler.planSchedule` now partitions by `ReminderRule.mode`: a `fixed` rule expands exactly as before and is never an input to the aggregate (which is how "a reminder somebody set by hand always fires" is guaranteed structurally), while a `smart` rule hands its time to the planner and produces nothing of its own. `buildAggregateCopy` over the `notify` namespace, injected at the composition root — and injecting it is what TURNS THE FEATURE ON, so with no builder a smart rule expands like a fixed one and behaviour is byte-identical to what shipped (that is a test, not a hope). Smart is a real, selectable mode in the Journey reminder editor: the time field is hidden (the app picks it) and the day picker stays (which days somebody is reachable is theirs to say). Suppression is implemented at the two places it is actually true — "already foregrounded" is EXACT, in the notification handler, which only runs in the foreground; "nothing pending" leaves a Journey with no pending Step out at plan time and lets the next reconcile cancel what was pending, an approximation whose remaining gap (finishing everything after the day's last reconcile) is now written down rather than discovered. |
| **R8** | **Every notification in the user's chosen voice** (D84) | ✅ **Built 2026-08-26** | Eight types × four voices × two languages authored against the register the questionnaire's own preview examples set, so what a person is shown while choosing is what they get. No mechanism was missing — `notificationContent` has resolved a `_<styleId>` suffix with a base fallback since D40. Two things stay untoned, deliberately: `journey_closed` (a warm spin on somebody else's setback is the forced positivity the product refuses — enforced structurally by `neverToned`, not by an absent key), and the in-app Activity list, which is a feed of what happened rather than a send. The test now states the promise: every toneable type resolves all four voices in both languages, none falls through to the base, and the four differ from EACH OTHER. |
| **R9** | **Personalized Motivation Engine** | ✅ **First slice built 2026-08-26** (the engine stays Future) | The Future PRD's 24 open questions are answered FOR THE FIRST SLICE in `PRD/Motivation_First_Slice_PRD.md`, and each one that stays open is marked. Shipped: a Home card, at most once a day, absent on most days, grounded in a number the app counted itself, with Helpful / Not helpful underneath. No push, no quotations, no health claims, no money figures — each excluded for a stated reason, not cut. Truth is structural: an item declares the facts its sentence needs, and an item missing one cannot be selected, so it can never be shown with an invented number. Feedback is one binary signal; "not now" is a dismissal and records no opinion; a dislike retires the item at that VERSION. The whole record is ids and verdicts, inside `AppState`, so export and deletion cover it for free. Zero cost, no backend, no new permission. **What is still Future:** push delivery, quotations, health statements, money metrics, cross-user ranking, "motivate me now", reasoned dislike, Dream-level metrics, a catalog service. |
| **R10** | **Onboarding Q6 → the Journey builder** (D82) | ⛔ Ready | The capacity answer currently reaches nothing. It belongs to the agent that BUILDS a Journey, so a person with an hour a week is not handed a plan that needs five. Read it through `toCoachSummary().capacity` in `goalSpecToJourney`'s constraint derivation, beside the interview's own timing answers — and let the Journey-specific interview stay authoritative where the two disagree, because it was asked about this Journey, now. |
| **R6** | **An Ally is TOLD when a Journey is paused or resumed** | ⛔ Approved 2026-08-25 (D79), not built | Today a paused Journey silently vanishes from an Ally's view and reappears on resume — the opposite of what was asked for. Rather than widening the four-field whitelist with a status (a permanent property of a shared object, readable at any time), the owner's pause/resume writes a small server-side EVENT that the Journey's Allies' bells read: actor, journey id, kind, timestamp. No reason, no note, no free text, and no change to what an Ally can see at rest. Needs: a migration + RLS (an Ally of that Journey may read its events), a gateway read, two new `NotificationKind`s, copy, and the negative test that nothing else rides the row. |
